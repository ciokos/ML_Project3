...
def dbplotf(X, y, fun, grid_range, resolution=100.0):
    ...
    if C == 3: C_colors = ['b', 'g', 'r']; C_legend = ['Class A (y=0)', 'Class B (y=1)', 'Class C (y=2)']; C_levels = [
        .66, 1.34]
    if C == 4: C_colors = ['b', 'w', 'y', 'r']; C_legend = ['Class A (y=0)', 'Class B (y=1)', 'Class C (y=2)',
                                                            'Class D (y=3)']; C_levels = [.74, 1.5, 2.26]
    if C > 4:
    ...
    if len(set(values_list)) == 1: raise ValueError(
    ...
    plt.contourf(F1, F2, values, levels=np.linspace(values.min(), values.max(), levels), cmap=plt.cm.jet,
                 origin='image')
...
def clusterval(y, clusterid):
    ...
    for k in range(K):
        m_i[k] = np.count_nonzero(clusterid == k)
        yk = y[clusterid == k]
        for c in range(C):
            m_ij = np.count_nonzero(yk.astype(int) == c) # number of objects of j'th class in i'th cluster
            m_ij = np.array(m_ij).astype(float)
            p_ij[k, c] = m_ij / m_i[k][0]
